...
public class Course implements Comparable<Course> {
  ...
  public Course(String deptName, int courseNum, int numCredits, int seats) throws IllegalArgumentException{
    if(deptName.isBlank() || deptName == null || courseNum <= 0 || numCredits < 1 || numCredits > 5 || seats < 0) {
      throw new IllegalArgumentException("Course could not be created as inputs are not valid");
    }
  ...
  public void setProfessor(String profName, double rating) throws IllegalArgumentException {
    if(profName != null && (rating > 5 || rating < 0)) {
      throw new IllegalArgumentException("rating is invalid and name is not null,");
    }
  ...
  public void setSeatsAvailable(int numSeatsAvailable) {
    if(numSeatsAvailable < 0) {
      throw new IllegalArgumentException("can not set seats available to negative");
    }
  ...
  public int compareTo(Course otherCourse) {
    ...
    if(this.DEPT_NAME.equals("CS") && !otherCourse.DEPT_NAME.equals("CS")) {// CS department first
      return 1;
    }
    else if(!this.DEPT_NAME.equals("CS") && otherCourse.DEPT_NAME.equals("CS")) {// CS department first
      return -1;
    }
    else {
      if(this.numSeatsAvailable > 0 && otherCourse.numSeatsAvailable == 0) {//seats available as long as not 0
        return 1;
      }
        ...
        if(this.profName != null && otherCourse.profName == null) {//if professor name exists
